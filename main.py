...
"""
    summary_template = """given the information about Elon Musk, summarize it in 3 sentences.    """

    summary_prompt_template = PromptTemplate(
        input_variables=["information"], template=summary_template
    )
    
    llm = ChatOllama(temperature=0, model="qwen2.5:1.5b")
    #llm = ChatGoogleGenerativeAI(temperature=0, model="gemini-2.0-flash", google_api_key=os.getenv("GOOGLE_API_KEY"))
    
    #llm = ChatOllama(temperature=0, model="deepseek-r1:1.5b")
    #llm = ChatOllama(temperature=0, model="medllama2:latest")
